...
Persistent export deliberately stores the typed snapshot as an ordinary column;
the next `apply` milestone will reuse the same raw/typed plan with a virtual
generated column, where DuckDB's binder-enforced read-only behavior is useful.
