...
The published MCP contract is covered through `tools/list`: paired preview/commit
tools expose no dynamic `write` switch, `apply` keeps the public `from` alias, and
`duckdb_export` preserves the CLI's structured collision payload.
