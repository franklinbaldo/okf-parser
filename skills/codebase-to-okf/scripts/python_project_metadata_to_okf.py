...
def _relative_link(from_path: str, to_path: str) -> str:
    """Return a POSIX relative link between two generated concept paths."""
    start = PurePosixPath(from_path).parent
    return str(PurePosixPath("..") / to_path) if start == PurePosixPath("project") else to_path
...
def _render_dependency(
    ...
    body = [
        ...
        "",
        "This concept proves only that the dependency is declared in project metadata. It does not ",
        "prove installation, import, reachability, or runtime use.",
    ]
    return f"---\n{_frontmatter(fields)}\n---\n\n{'\n'.join(body)}\n"
...
def _clear_previous(root: Path) -> None:
    ...
    for directory, expected_type in (("project", PROJECT_TYPE), ("dependencies", DEPENDENCY_TYPE)):
...
def _write_projection(source: Path, root: Path) -> tuple[int, int, list[str]]:
    ...
    project_filename = _stable_filename("project", project_name, f"{MANIFEST_NAME}:{project_name}")
    ...
    for record in dependencies:
        ...
        (root / relative).write_text(
            _render_dependency(relative, project_name, project_path, record),
            encoding="utf-8",
        )
    ...
    (root / project_path).write_text(
        _render_project(project_path, project, dependencies, tuple(dependency_paths)),
        encoding="utf-8",
    )
    groups = list(dict.fromkeys(record.group for record in dependencies))
