...
def _load_concept(
    ...
    record = ConceptRecord(
        concept_id=doc_id,
        logical_key=f"{root.as_posix()}::{doc_id}",
        path=relative,
...
def load_bundle(root: Path, *, require_all_caps_frontmatter: bool = False) -> Bundle:
    ...
    paths = sorted(
        ...
        if _IGNORED_DIRECTORIES.isdisjoint(path.relative_to(root).parts)
    )
        ...
        if is_reserved_document(path):
            ...
            if path.name == "index.md":
                ...
                try:
                    ...
                    if require_all_caps_frontmatter and frontmatter is None:
                        reserved_diagnostics.append(
                            Violation(
                                "OKF011",
                                Severity.ERROR,
                                relative,
                                "ALL-CAPS Markdown must contain frontmatter under current policy",
                            )
                        )
                except DocumentParseError as exc:
                    body = text
                    reserved_diagnostics = [Violation("OKF010", Severity.ERROR, relative, str(exc))]
...
def validate_path(
    ...
    *,
    ...
    bundle = load_bundle(path, require_all_caps_frontmatter=require_all_caps_frontmatter)
