...
from okf_tools.models import ParsedDocument
...
def is_reserved_document(path: Path) -> bool:
    ...
    stem = path.stem
    return path.name in RESERVED_FILENAMES or (
        stem == stem.upper() and any(character.isalpha() for character in stem)
    )
...
def iter_markdown_links(body: str) -> list[str]:
    ...
    return [match.group(1) for match in _MARKDOWN_LINK_RE.finditer(body)]
